...
<body>
  ...
  <div style="display: flex; align-items: center; gap: 24px; margin-bottom: 1.5rem;">
    ...
    <div>
      ...
      <div style="margin-bottom:0;">
        <span class="info-label">Address:</span> {{ $profil[0]['address'] ?? 'Jakarta, Indonesia' }}<br>
        <span class="info-label">Phone:</span> {{ $profil[0]['phone'] ?? '0812-3456-7890' }}<br>
        <span class="info-label">Email:</span> {{ $profil[0]['email'] ?? 'nama@email.com' }}<br>
        <span class="info-label">LinkedIn:</span> {{ $profil[0]['linkedin'] ?? 'LinkedIn Profile URL' }}<br>
        <span class="info-label">Website:</span> {{ $profil[0]['portfolio'] ?? 'Portfolio/Website URL' }}
      </div>
    </div>
  </div>
    ...
    @foreach ($pengalamankerja as $item)
      <div class="mb-3">
        <div class="job-header">
          ...
          <span>{{ $item['jobStartDate'] ?? 'Jan 2024' }} - {{ $item['isPresent'] ? 'Sekarang' : ($item['jobEndDate'] ?? 'Jan 2025') }}</span>
        </div>
  ...
  <ul class="mb-2">
    <li><b>Bahasa:</b> {{ !empty($bahasa) ? implode(', ', $bahasa) : 'English, French, Mandarin' }}</li>
    <li><b>Sertifikat:</b> {{ !empty($sertifikat) ? implode(', ', $sertifikat) : 'Professional Design Engineer (PDE) License, Project Management Tech (PMT), Structural Process Design (SPD)' }}</li>
    <li><b>Hobi:</b> {{ !empty($hobi) ? implode(', ', $hobi) : 'Tenis Lapangan' }}</li>
  </ul>
</body>
</html>
